...
def view_exams(request):
    ...
    new_Form = ExamForm()
    new_Form.fields["section"].queryset = Section.objects.filter(
        professor=prof) #student_group
...
def view_exam(request, exam_id):
    ...
    return render(request, 'prof/exam/view_exam.html', {
        'exam': exam, 'prof': prof, 'student_group': exam.section.all()   #student group needs to b3 changed
    })
...
def edit_exam(request, exam_id):
    ...
    new_Form.fields["section"].queryset = Section.objects.filter(professor=prof) #student_group
