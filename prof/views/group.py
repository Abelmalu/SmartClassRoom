...
def create_student_group(request):
    ...
    return render(request, 'prof/group/addview_groups.html', {
        'special_students_db': Section.objects.filter(professor=prof), 'prof': prof, 'groupForm': Group_Form()
    })
    

def view_specific_group(request, group_id):
    prof = request.user
    group = Section.objects.get(professor=prof, pk=group_id)
...
def delete_group(request, group_id):
    prof = request.user
    Section.objects.filter(professor=prof, pk=group_id).delete()
    return redirect('prof:view_groups')
